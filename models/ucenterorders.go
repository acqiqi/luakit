...
type UcenterOrders struct {
	...
	MainOrderId    int     `int:"main_order_id"`    // 关联订单id
	TotalLinkPrice float64 `int:"total_link_price"` // 所有关联订单金额
...
func GetLinkOrders(order_id int64) (row int64, v *[]UcenterOrders, err error) {
	o := orm.NewOrm()
	v = &[]UcenterOrders{}
	if row, err = o.QueryTable(new(UcenterOrders)).Filter("MainOrderId", order_id).Filter("Status__gt", 1).Filter("IsAccounts", 0).Filter("flag", 1).RelatedSel().All(v); err == nil {
		return row, v, nil
	}
	return 0, nil, err
}
